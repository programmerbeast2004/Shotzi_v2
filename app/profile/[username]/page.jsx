...
export default function UserProfileByUsernamePage() {
  ...
  // Support both [username] and [slug] parameter naming
  const usernameParam = params?.username || params?.slug;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // 1. Try finding profile by username
      ...
      // 3. Fallback: Check if author exists in posts
      if (!prof) {
        ...
        if (isUuid) {
          query = query.eq("user_id", usernameParam);
        } else {
          query = query.ilike("user_email", `${usernameParam}@%`);
        }
  ...
  if (!currentUser) {
    return (
      <AuthBarrier
        ...
        subtitle={`Sign in to view @${usernameParam || 'photographer'}'s unfiltered moments, follow their journey, and connect.`}
      ...
      <div className="py-16 max-w-md mx-auto text-center">
        <div className="rounded-3xl border-[2.5px] border-black bg-white p-8 shadow-[6px_6px_0px_#18181B] space-y-4">
          ...
          <p className="text-xs text-zinc-600 font-medium">
            This photographer hasn't published any moments or may have scootered away.
          </p>
          <Link
            href="/"
